...
void addTask() {
  ...
  try {
    ...
    if (savedTasksFile.lengthSync() == 0) {
      savedTasksFile.writeAsStringSync('$addedTaskContent',
...
void editTask() {
  ...
  if (editedTaskIndex == null ||
      editedTaskIndex < 1 ||
      editedTaskIndex > savedTasks.length) {
...
void deleteTask() {
  displayTasks();
  String tasksToDeleteInput =
      getInput("Enter task number to delete(comma or space-separated):   ");
  ...
  if (deletedTaskIndex.contains(null)) {
    forwardTo(
        "Error: Invalid input! Please enter valid task number(s).", deleteTask);
    return;
  }
  ...
  saveTasksToFile(
      successMessage: "Task(s) has been deleted successfully",
...
//An ANSI code which clears the console a.k.a terminal.
...
String getInput(String prompt) {
  ...
  return input ??
      "Error: Invalid input! Perhaps you left it empty or typed something incorrect.";
  //It just returns the second value if it is null.
}
